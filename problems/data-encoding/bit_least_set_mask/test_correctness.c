...
{
  fprintf(stderr, "bit_least_set_mask(%d) failed (expected %d, got %d)\n", x,
...
int test_case(int x)
{
  ...
  int test_result = bit_least_set_mask(x);
  if (expected_result != test_result) {
    print_error(x, expected_result, test_result);
  }
  return expected_result == bit_least_set_mask(x);
}
